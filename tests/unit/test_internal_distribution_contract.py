...
class InternalDistributionContractTest(unittest.TestCase):
    ...
    def test_setup_script_collects_distribution_proof_inputs(self):
        ...
        self.assertIn("repository + production-environment state only", SETUP_SCRIPT)
        self.assertIn("Release workflow is still blocked in repo/production scopes. Missing required secrets:", SETUP_SCRIPT)
        self.assertIn("Ready for repo/production scopes: $SECRETS_SET secrets configured and required checks passed.", SETUP_SCRIPT)
        self.assertIn("verify org-level Actions secrets/vars separately", SETUP_SCRIPT)
