import Foundation
import Combine
// import RevenueCat // Temporarily disabled for CI
...
#if false // Temporarily disabled for CI - RevenueCat not available
...
// MARK: - PurchasesDelegate (RevenueCat integration disabled for CI)
...
#else // CI stub implementation

/// Stub subscription service for CI builds
@available(iOS 17.0, *)
@Observable
public final class SubscriptionService: NSObject {
    @MainActor public static let shared = SubscriptionService()

    public private(set) var subscriptionStatus = SubscriptionStatus(tier: .free, isActive: false)
    public private(set) var isLoading = false
    ...
    public func initializeRevenueCat() async {}
    public func checkSubscriptionStatus() async {}
    public func hasAccess(to feature: String) -> Bool { return false }
    public func getCurrentTier() -> SubscriptionTier { return .free }
    public func purchaseProSubscription(yearly: Bool) async -> PurchaseResult { return .error("Not available in CI") }
    public func restorePurchases() async -> PurchaseResult { return .error("Not available in CI") }
    public func checkProFeatureAccess(feature: String) -> Bool { return false }
}
