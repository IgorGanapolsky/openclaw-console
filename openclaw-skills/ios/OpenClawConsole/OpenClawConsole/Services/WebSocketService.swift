...
final class WebSocketService: NSObject {
    ...
    private func parseMessage(_ text: String) {
        ...
        case .gitStateChanged:
            // Parse git state change event
            if let rawData = try? JSONSerialization.jsonObject(with: payloadData) as? [String: Any],
               let agentId = rawData["agentId"] as? String {
                // Try to decode GitState, use placeholder if fails
                if let gitStateData = rawData["gitState"] as? [String: Any],
                   let gitStateJsonData = try? JSONSerialization.data(withJSONObject: gitStateData),
                   let gitState = try? JSONDecoder().decode(GitState.self, from: gitStateJsonData) {
                    event = .gitStateChanged(agentId, gitState)
                } else {
                    // Create a minimal GitState placeholder for unparseable data
                    let placeholderState = GitState(
                        repository: "unknown",
                    ...
                    event = .gitStateChanged(agentId, placeholderState)
                }
            }
        }
