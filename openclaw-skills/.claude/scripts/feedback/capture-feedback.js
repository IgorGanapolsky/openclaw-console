...
function createFeedbackEntry(args) {
  ...
  // Rubric-based scoring (opt-in via --rubric flag)
  if (args.rubric && args['rubric-scores']) {
    const rubricEval = parseRubricScores(args['rubric-scores']);
    entry.rubricEvaluation = rubricEval;
    // Continuous reward: map 0-1 aggregate to -1..+1
    entry.rubricReward = rubricEval.aggregate * 2 - 1;
  }
...
/**
 * Parse rubric scores from CLI string and evaluate with AGENT_PERFORMANCE_RUBRIC
 * @param {string} scoresStr - e.g. "taskCompletion:4,minimalDiff:5,conventionAdherence:3,verification:4,communication:5"
 * @returns {{ dimensions: object, aggregate: number, report: string }}
 */
function parseRubricScores(scoresStr) {
  const scores = {};
  scoresStr.split(',').forEach(pair => {
    const [dim, val] = pair.trim().split(':');
    if (dim && val !== undefined) {
      scores[dim.trim()] = parseFloat(val);
    }
  });

  const result = evaluateWithRubric(AGENT_PERFORMANCE_RUBRIC, scores);
  const report = formatRubricReport(AGENT_PERFORMANCE_RUBRIC, scores, result);
  return { dimensions: scores, aggregate: result.aggregate, report };
}
...
function exportTrainingData() {
  ...
  // Load raw feedback for rubric enrichment
  const rawFeedback = fs.existsSync(FEEDBACK_FILE)
    ? fs.readFileSync(FEEDBACK_FILE, 'utf8').trim().split('\n').filter(Boolean).map(l => JSON.parse(l))
    : [];
  const rubricMap = {};
  rawFeedback.forEach(f => {
    if (f.rubricEvaluation) rubricMap[f.id] = f.rubricEvaluation;
  });
  ...
  const pytorchData = {
    metadata: {
      ...
      features: ['rewardSequence', 'recentTrend', 'timeGaps', 'rubricScores']
    },
    sequences: sequences.map(s => ({
      ...
      y: s.targetReward,
      label: s.label,
      rubric: s.rubricEvaluation || null,
    }))
  };
...
function displayStats(summary) {
  ...
  // Rubric averages (when rubric data exists)
  if (fs.existsSync(FEEDBACK_FILE)) {
    const allFeedback = fs.readFileSync(FEEDBACK_FILE, 'utf8').trim().split('\n').filter(Boolean).map(l => JSON.parse(l));
    const rubricEntries = allFeedback.filter(f => f.rubricEvaluation);
    if (rubricEntries.length > 0) {
      console.log(`\n📋 Rubric Averages (${rubricEntries.length} entries):`);
      const dimTotals = {};
      const dimCounts = {};
      rubricEntries.forEach(f => {
        for (const [dim, val] of Object.entries(f.rubricEvaluation.dimensions)) {
      ...
      for (const dim of Object.keys(dimTotals)) {
        ...
        console.log(`   ${dim.padEnd(18)} avg: ${avg.toFixed(2)}`);
      }
      const avgAggregate = rubricEntries.reduce((s, f) => s + f.rubricEvaluation.aggregate, 0) / rubricEntries.length;
      console.log(`   ${'aggregate'.padEnd(18)} avg: ${avgAggregate.toFixed(2)}`);
    }
  }
...
Options:
  ...
  --rubric         Enable rubric-based scoring (uses AGENT_PERFORMANCE_RUBRIC)
  --rubric-scores  Dimension scores, e.g. "task-completion:4,minimal-diff:5,convention:3,verification:4,communication:5"
...
`);
  ...
  try {
    ...
    if (entry.rubricEvaluation) {
      console.log(`   Rubric Aggregate: ${entry.rubricEvaluation.aggregate.toFixed(2)}`);
      console.log(`   Rubric Reward: ${entry.rubricReward.toFixed(2)}`);
    }
